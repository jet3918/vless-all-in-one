...
readonly SCRIPT_RAW_URL="https://raw.githubusercontent.com/jet3918/vless-all-in-one/main/vless-server.sh"
...
# 个人一键脚本安全设置：保留原有容错行为，不启用完整 set -euo pipefail
umask 077
readonly DB_LOCK_FILE="$CFG/.database.lock"
...
ensure_secure_permissions() {
    install -d -m 700 "$CFG" "$CFG/cache" "$VERSION_CACHE_DIR" 2>/dev/null || return 1
    [[ -f "$DB_FILE" ]] && chmod 600 "$DB_FILE" 2>/dev/null || true
    [[ -f "$TG_CONFIG_FILE" ]] && chmod 600 "$TG_CONFIG_FILE" 2>/dev/null || true
    find "$CFG" -maxdepth 3 -type f \
        \( -name '*.key' -o -name '*.join' -o -name 'warp*.conf' -o -name 'warp.json' \) \
...
acquire_instance_lock() {
    ...
    if command -v flock >/dev/null 2>&1; then
        ...
        trap 'rmdir "$DB_LOCK_DIR" 2>/dev/null || true' EXIT
    fi
}

# 清理旧版本生成的重复 Watchdog；仅匹配本脚本原有的固定内容和服务文件。
cleanup_legacy_watchdog() {
    local changed=false
    local systemd_unit="/etc/systemd/system/vless-watchdog.service"
    local openrc_unit="/etc/init.d/vless-watchdog"
    local watchdog_script="$CFG/watchdog.sh"

    if [[ -f "$systemd_unit" ]] && grep -Fq "$watchdog_script" "$systemd_unit" 2>/dev/null; then
        systemctl disable --now vless-watchdog 2>/dev/null || true
        rm -f -- "$systemd_unit"
        changed=true
    fi

    if [[ -f "$openrc_unit" ]] && grep -Fq "$watchdog_script" "$openrc_unit" 2>/dev/null; then
        rc-service vless-watchdog stop 2>/dev/null || true
        rc-update del vless-watchdog default 2>/dev/null || true
        rm -f -- "$openrc_unit"
        changed=true
    fi

    if [[ -f "$watchdog_script" ]] && grep -Fq 'vless-watchdog.log' "$watchdog_script" 2>/dev/null; then
        rm -f -- "$watchdog_script"
        changed=true
    fi

    if [[ "$changed" == "true" ]]; then
        rm -f -- /var/log/vless-watchdog.log 2>/dev/null || true
        command -v systemctl >/dev/null 2>&1 && systemctl daemon-reload 2>/dev/null || true
    fi
}

# 记录脚本新创建的系统文件；重复记录自动跳过
record_managed_path() {
    local path="$1"
    [[ -z "$path" || "$path" != /* ]] && return 1
    install -d -m 700 "$CFG" 2>/dev/null || return 1
    touch "$MANIFEST_FILE" || return 1
    chmod 600 "$MANIFEST_FILE" 2>/dev/null || true
    grep -Fxq -- "$path" "$MANIFEST_FILE" 2>/dev/null || printf '%s\n' "$path" >> "$MANIFEST_FILE"
}

# 仅当目标原本不存在时认领，避免卸载误删用户原有文件
prepare_managed_path() {
    local path="$1"
    [[ -e "$path" || -L "$path" ]] && return 0
    record_managed_path "$path"
}

remove_managed_path() {
    local path="$1"
    [[ -f "$MANIFEST_FILE" ]] || return 0
    grep -Fxq -- "$path" "$MANIFEST_FILE" 2>/dev/null || return 0
    ...
    local tmp
    tmp=$(mktemp "$CFG/.manifest.XXXXXX") || return 1
    grep -Fvx -- "$path" "$MANIFEST_FILE" > "$tmp" 2>/dev/null || true
...
install_managed_file() {
    ...
    prepare_managed_path "$dest" || return 1
    install -m "$mode" "$src" "$dest"
}

create_managed_symlink() {
    local target="$1" link="$2"
    prepare_managed_path "$link" || return 1
    ln -sfn "$target" "$link"
}

# 卸载时只删除清单内、且位于允许目录中的路径
remove_managed_paths() {
    [[ -f "$MANIFEST_FILE" ]] || {
        echo "  未找到安装清单，跳过系统文件删除，避免误删现有程序。"
        return 0
    }

    local reversed path
    if command -v tac >/dev/null 2>&1; then
        reversed=$(tac "$MANIFEST_FILE")
    else
        reversed=$(awk '{line[NR]=$0} END {for (i=NR;i>=1;i--) print line[i]}' "$MANIFEST_FILE")
    fi

    while IFS= read -r path; do
        [[ -z "$path" || "$path" == *'..'* ]] && continue
        case "$path" in
            "$CFG"/*|/usr/local/bin/*|/usr/local/share/xray|/usr/local/share/xray/*|/usr/bin/vless|\
            /etc/systemd/system/vless-*.service|/etc/systemd/system/fake-web.service|\
            /etc/init.d/vless-*|/etc/nginx/sites-available/vless-*|/etc/nginx/sites-enabled/vless-*|\
            /etc/nginx/sites-available/xhttp-cdn|/etc/nginx/sites-enabled/xhttp-cdn|\
            /etc/nginx/conf.d/vless-*|/etc/nginx/conf.d/xhttp-cdn.conf|\
            /etc/nginx/http.d/vless-*|/etc/nginx/http.d/xhttp-cdn.conf|/var/www/html/index.html)
                if [[ -d "$path" && ! -L "$path" ]]; then
                    rm -rf -- "$path"
                else
                    rm -f -- "$path"
                ...
                ;;
            *)
                echo "  跳过不在允许范围的清单路径: $path" >&2
                ;;
        esac
    done <<< "$reversed"
}

# 下载文件基础校验：非空、拒绝 HTML 错误页、校验压缩包或 ELF/脚本格式
verify_download_basic() {
    local file="$1" kind="${2:-auto}" expected_sha256="${3:-}"
    [[ -s "$file" ]] || { echo "下载文件为空: $file" >&2; return 1; }

    local size
    size=$(wc -c < "$file" 2>/dev/null || echo 0)
    [[ "$size" -ge 1024 ]] || { echo "下载文件过小: $file ($size bytes)" >&2; return 1; }

    if head -c 512 "$file" 2>/dev/null | tr '[:upper:]' '[:lower:]' | \
        grep -Eq '<!doctype html|<html|access denied|bad gateway'; then
        echo "下载结果疑似网页错误内容: $file" >&2
        return 1
    fi

    if [[ -n "$expected_sha256" ]]; then
        command -v sha256sum >/dev/null 2>&1 || { echo "缺少 sha256sum，无法校验" >&2; return 1; }
        local actual_sha256
    ...
    case "$kind" in
        ...
        tar.xz|txz) tar -tJf "$file" >/dev/null 2>&1 ;;
        script)
            head -n 1 "$file" | grep -q '^#!' && bash -n "$file" >/dev/null 2>&1
            ;;
        binary)
            [[ "$(head -c 4 "$file" 2>/dev/null | od -An -tx1 | tr -d ' \n')" == "7f454c46" ]]
            ;;
        auto)
            return 0
            ;;
...
download_file_checked() {
    local url="$1" output="$2" kind="${3:-auto}" expected_sha256="${4:-}"
    rm -f -- "$output"
    curl --fail --location --silent --show-error --retry 3 --retry-delay 2 \
        --connect-timeout 15 --max-time 180 --proto '=https' --tlsv1.2 \
        -o "$output" "$url" || return 1
    verify_download_basic "$output" "$kind" "$expected_sha256" || {
        rm -f -- "$output"
        return 1
    }
}
...
_db_touch() {
    ...
    # Alpine busybox date 不支持 -Iseconds，使用兼容格式
    now=$(date '+%Y-%m-%dT%H:%M:%S%z' 2>/dev/null || date '+%Y-%m-%dT%H:%M:%S')
    tmp=$(mktemp "$CFG/.db.touch.XXXXXX") || return 1
...
_db_apply() { # _db_apply [jq args...] 'filter'
    [[ -f "$DB_FILE" ]] || init_db || return 1
    local tmp; tmp=$(mktemp "$CFG/.db.apply.XXXXXX") || return 1
...
db_add_ip_routing_rule() {
    ...
    local tmp
    tmp=$(mktemp "$CFG/.telegram.XXXXXX") || return 1
...
# 兼容旧调用：Telegram 配置统一使用 $TG_CONFIG_FILE，不再写入 db.json
db_get_tg_config() { tg_get_config "$1"; }
db_set_tg_config() { tg_set_config "$1" "$2"; }
send_tg_message() { tg_send_message "$1"; }
...
# 初始化 TG 配置
init_tg_config() {
    [[ -f "$TG_CONFIG_FILE" ]] && { chmod 600 "$TG_CONFIG_FILE" 2>/dev/null || true; return 0; }
    install -d -m 700 "$CFG" || return 1
...
xray_api_query() {
    ...
    local args=(api statsquery "--server=127.0.0.1:${XRAY_API_PORT}")
...
# 保存 JOIN 信息到文件
# 用法: _save_join_info "协议名" "数据格式" "链接函数" [链接参数...]
# 链接参数中的 %s/%b 会替换为当前 IPv4/IPv6 地址。
_save_join_info() {
    local protocol="$1" data_fmt="$2" link_fn="$3"; shift 3
    local join_file="$CFG/${protocol}.join"
    local link_prefix; link_prefix=$(tr '[:lower:]-' '[:upper:]_' <<<"$protocol")
    local link_args=("$@")

    declare -F "$link_fn" >/dev/null 2>&1 || {
        _err "未知链接生成函数: $link_fn"
        return 1
    }
    ...
    local label ip ipfmt data code link arg resolved_arg
    for label in V4 V6; do
        ...
        for arg in "${link_args[@]}"; do
            resolved_arg=${arg//%s/$ipfmt}
            resolved_arg=${resolved_arg//%b/$ipfmt}
            resolved_args+=("$resolved_arg")
        done
        link=$("$link_fn" "${resolved_args[@]}") || return 1
...
_ensure_nginx_https_for_reality() {
    ...
    prepare_managed_path "$nginx_conf" || return 1
    cat > "$nginx_conf" << EOF
...
EOF
    ...
    if [[ "$nginx_conf" == *"sites-available"* ]]; then
        create_managed_symlink "$nginx_conf" "/etc/nginx/sites-enabled/vless-reality-https" 2>/dev/null
    fi
...
NGINX_EOF
    ...
    # 如果是 sites-available 模式，创建软链接
    if [[ "$nginx_conf" == *"sites-available"* ]]; then
        create_managed_symlink "$nginx_conf" "/etc/nginx/sites-enabled/xhttp-cdn" 2>/dev/null
    fi
...
force_cleanup() {
    # 停止所有 vless 相关服务
    local services="reality hy2 tuic snell snell-v5 anytls singbox"
    ...
    # 服务均由上面的明确服务名停止，不再 killall 同名进程，避免影响其他实例。
...
cleanup_hy2_nat_rules() {
    ...
    # 只清理本脚本带标记的端口跳跃规则，不碰其他程序的 REDIRECT 规则
    local tool chain marker
    for tool in iptables ip6tables; do
        command -v "$tool" >/dev/null 2>&1 || continue
        for chain in PREROUTING OUTPUT; do
            for marker in vless-hy2-hop vless-tuic-hop; do
                "$tool" -t nat -S "$chain" 2>/dev/null | while IFS= read -r rule; do
                    [[ "$rule" == *"--comment "$marker""* || "$rule" == *"--comment $marker"* ]] || continue
                    local rule_parts=()
                    read -r -a rule_parts <<< "$rule"
                    [[ "${rule_parts[0]:-}" == "-A" ]] || continue
                    rule_parts[0]="-D"
                    "$tool" -t nat "${rule_parts[@]}" 2>/dev/null || true
                done
            done
        done
    done
...
create_fake_website() {
    ...
    # 删除旧配置，确保使用最新配置
    remove_managed_path "$nginx_conf_file"
    remove_managed_path /etc/nginx/sites-enabled/vless-fake
    # 只清理本脚本创建的订阅配置，避免删除其他程序文件
    remove_managed_path /etc/nginx/conf.d/vless-sub.conf
    remove_managed_path /etc/nginx/http.d/vless-sub.conf
    
    # 创建网页目录
    mkdir -p "$web_dir"
    
    # 创建简单的伪装网页
    prepare_managed_path "$web_dir/index.html" || return 1
...
EOF
    ...
    if [[ -n "$domain" ]] && [[ -f "/etc/vless-reality/certs/server.crt" ]]; then
        ...
        prepare_managed_path "$nginx_conf_file" || return 1
...
EOF
        ...
        if [[ "$nginx_conf_dir" == "/etc/nginx/sites-available" ]]; then
            ...
            create_managed_symlink "$nginx_conf_file" /etc/nginx/sites-enabled/vless-fake
        fi
...
install_acme_tool() {
    ...
    # 方法1: 下载后校验语法再执行，避免 curl | sh
    local acme_tmp
    acme_tmp=$(mktemp "${TMPDIR:-/tmp}/vless-acme.XXXXXX") || return 1
    if download_file_checked "https://raw.githubusercontent.com/acmesh-official/acme.sh/master/acme.sh" "$acme_tmp" script; then
        if bash "$acme_tmp" --install -m "$ACME_DEFAULT_EMAIL" 2>&1 | grep -qE "Install success|already installed"; then
            rm -f -- "$acme_tmp"
            source "$HOME/.acme.sh/acme.sh.env" 2>/dev/null || true
            [[ -f "$HOME/.acme.sh/acme.sh" ]] && { _ok "acme.sh 安装成功"; return 0; }
        fi
    fi
    rm -f -- "$acme_tmp"
    
    # 方法2: 使用随机临时目录 git clone
    if command -v git &>/dev/null; then
        _info "尝试使用 git 安装..."
        local acme_git_tmp
        acme_git_tmp=$(mktemp -d "${TMPDIR:-/tmp}/vless-acme-git.XXXXXX") || return 1
        if git clone --depth 1 https://github.com/acmesh-official/acme.sh.git "$acme_git_tmp" 2>/dev/null; then
            (cd "$acme_git_tmp" && ./acme.sh --install -m "$ACME_DEFAULT_EMAIL" 2>/dev/null)
            rm -rf -- "$acme_git_tmp"
            [[ -f "$HOME/.acme.sh/acme.sh" ]] && { _ok "acme.sh 安装成功 (git)"; return 0; }
        fi
        rm -rf -- "$acme_git_tmp"
    fi
    
    # 方法3: 直接下载到目标前先在临时文件中校验
    _info "尝试直接下载..."
    mkdir -p "$HOME/.acme.sh"
    local acme_direct_tmp
    acme_direct_tmp=$(mktemp "${TMPDIR:-/tmp}/vless-acme-direct.XXXXXX") || return 1
    if download_file_checked "https://raw.githubusercontent.com/acmesh-official/acme.sh/master/acme.sh" "$acme_direct_tmp" script; then
        install -m 700 "$acme_direct_tmp" "$HOME/.acme.sh/acme.sh"
        rm -f -- "$acme_direct_tmp"
        [[ -f "$HOME/.acme.sh/acme.sh" ]] && { _ok "acme.sh 安装成功 (直接下载)"; return 0; }
    fi
    rm -f -- "$acme_direct_tmp"
...
_issue_cert_dns() {
    ...
    # 设置 DNS API 环境变量，不使用 eval
    case "$dns_choice" in
        1) export CF_Token="$cf_token" ;;
        2) export Ali_Key="$ali_key" Ali_Secret="$ali_secret" ;;
        3) export DP_Id="$dp_id" DP_Key="$dp_key" ;;
    esac
    ...
    if "$acme_sh" --issue -d "$domain" --dns "$dns_api" --force 2>&1 | tee "$acme_dns_log" | grep -E "^\[|Verify finished|Cert success|error|Error" | sed 's/^/  /'; then
        ...
        echo ""
        _err "DNS 验证失败！"
        cat "$acme_dns_log" 2>/dev/null | grep -E "(error|Error)" | head -3
        rm -f -- "$acme_dns_log"
        return 1
    fi
}
...
_fetch_script_tmp() {
    ...
    local tmp_file
    tmp_file=$(mktemp "${TMPDIR:-/tmp}/vless-script.XXXXXX") || return 1
    if ! curl --fail --location --silent --show-error --retry 2         --connect-timeout "$connect_timeout" --max-time "$max_time"         --proto '=https' --tlsv1.2 -o "$tmp_file" "$SCRIPT_RAW_URL"; then
        rm -f -- "$tmp_file"
        return 1
    fi
    if ! verify_download_basic "$tmp_file" script; then
        rm -f -- "$tmp_file"
        return 1
    fi
    echo "$tmp_file"
...
_download_script_to() {
    local target="$1"
    prepare_managed_path "$target" || return 1
    local tmp_file
...
# 通用二进制下载安装函数
# 参数: name repo url_pattern package_kind binary_name [channel] [force] [version_override] [extra_binary]
_install_binary() {
    local name="$1" repo="$2" url_pattern="$3" package_kind="$4" binary_name="$5"
    local channel="${6:-stable}" force="${7:-false}" version_override="${8:-}" extra_binary="${9:-}"
    ...
    local tmp url src extra_src
    tmp=$(mktemp -d "${TMPDIR:-/tmp}/vless-download.XXXXXX") || return 1
    ...
    if ! download_file_checked "$url" "$tmp/pkg" "$package_kind"; then
        rm -rf -- "$tmp"
        _err "下载或校验 $name 失败: $url"
        return 1
    fi

    case "$package_kind" in
        zip) unzip -oq "$tmp/pkg" -d "$tmp/" || { rm -rf -- "$tmp"; return 1; } ;;
        tar.gz|tgz) tar -xzf "$tmp/pkg" -C "$tmp/" || { rm -rf -- "$tmp"; return 1; } ;;
        binary) ;;
        *) rm -rf -- "$tmp"; _err "不支持的包类型: $package_kind"; return 1 ;;
    esac

    if [[ "$package_kind" == "binary" ]]; then
        src="$tmp/pkg"
    else
        src=$(find "$tmp" -type f -name "$binary_name" -print -quit)
    fi
    [[ -n "$src" && -f "$src" ]] || { rm -rf -- "$tmp"; _err "未找到 $binary_name"; return 1; }

    install_managed_file "$src" "/usr/local/bin/$binary_name" 755 || { rm -rf -- "$tmp"; return 1; }
    if [[ -n "$extra_binary" ]]; then
        extra_src=$(find "$tmp" -type f -name "$extra_binary" -print -quit)
        [[ -n "$extra_src" ]] && install_managed_file "$extra_src" "/usr/local/bin/$extra_binary" 755 || true
    fi

    if [[ "$binary_name" == "xray" ]]; then
        prepare_managed_path "/usr/local/share/xray" || true
...
install_xray() {
    ...
    _install_binary "xray" "XTLS/Xray-core" \
        ...
        zip xray "$channel" "$force" "$version_override"
}
...
install_singbox() {
    ...
    _install_binary "sing-box" "SagerNet/sing-box" \
        ...
        tar.gz sing-box "$channel" "$force" "$version_override"
}
...
create_singbox_service() {
    ...
    if [[ "$DISTRO" == "alpine" ]]; then
        ...
        prepare_managed_path "/etc/init.d/$service_name" || return 1
...
EOF
        ...
        prepare_managed_path "/etc/systemd/system/${service_name}.service" || return 1
...
install_snell() {
    ...
    _info "安装 Snell v4..."
    local tmp=$(mktemp -d)
    if download_file_checked "https://dl.nssurge.com/snell/snell-server-v4.1.1-linux-${sarch}.zip" "$tmp/snell.zip" zip; then
        unzip -oq "$tmp/snell.zip" -d "$tmp/" && install_managed_file "$tmp/snell-server" /usr/local/bin/snell-server 755
...
install_snell_v5() {
    ...
    local tmp=$(mktemp -d)
    if download_file_checked "https://dl.nssurge.com/snell/snell-server-v${version}-linux-${sarch}.zip" "$tmp/snell.zip" zip; then
        unzip -oq "$tmp/snell.zip" -d "$tmp/" && install_managed_file "$tmp/snell-server" /usr/local/bin/snell-server-v5 755
        # Alpine: 解压 UPX 压缩 (Snell 官方二进制使用 UPX，musl 不兼容 UPX stub)
        if [[ "$DISTRO" == "alpine" ]] && command -v upx &>/dev/null; then
...
install_anytls() {
    ...
    _install_binary "anytls-server" "anytls/anytls-go" \
        ...
        zip anytls-server stable false "" anytls-client
}
...
install_naive() {
    ...
    if ! download_file_checked "$download_url" "$tmp/caddy.tar.xz" tar.xz; then
        _err "下载失败"
        rm -rf "$tmp"
        return 1
    fi
    ...
    if [[ -n "$caddy_bin" ]] && [[ -f "$caddy_bin" ]]; then
        ...
        if [[ "$magic" == "7f454c46" ]]; then
            chmod +x "$caddy_bin"
            install_managed_file "$caddy_bin" /usr/local/bin/caddy 755
            rm -rf "$tmp"
...
gen_hy2_server_config() {
    ...
    _save_join_info "hy2" "HY2|%s|$port|$password|$sni" \
        gen_hy2_link "%s" "$port" "$password" "$sni"
...
gen_tuic_server_config() {
    ...
    _save_join_info "tuic" "TUIC|%s|$port|$uuid|$password|$sni" \
        gen_tuic_link "%s" "$port" "$uuid" "$password" "$sni"
...
EOF
    ...
    prepare_managed_path /var/www/html/index.html || return 1
...
create_server_scripts() {
    # 使用 systemd/OpenRC 自身的 Restart 机制，不再生成重复 watchdog。
...
create_service() {
    ...
    _write_openrc() { # name desc cmd args [env]
        ...
        prepare_managed_path "/etc/init.d/${name}" || return 1
...
EOF
    ...
    _write_systemd() { # name desc exec pre before env [requires] [after]
        ...
        prepare_managed_path "/etc/systemd/system/${name}.service" || return 1
...
_auto_update_system_script() {
    ...
    prepare_managed_path "$system_script" || return 1
    ...
    if [[ -n "$real_path" && -f "$real_path" && "$real_path" != "$system_script" ]]; then
        ...
        if [[ "$need_update" == "true" ]]; then
            ...
            create_managed_symlink "$system_script" /usr/local/bin/vless 2>/dev/null
            create_managed_symlink "$system_script" /usr/bin/vless 2>/dev/null
            hash -r 2>/dev/null
...
remove_shortcut() {
    remove_managed_path /usr/local/bin/vless
    remove_managed_path /usr/bin/vless
    remove_managed_path /usr/local/bin/vless-server.sh
    _ok "快捷命令已移除"
}
...
download_wgcf() {
    ...
    # 仅替换由本脚本管理的旧文件，避免覆盖用户手动安装的 wgcf
    if [[ -f "/usr/local/bin/wgcf" ]]; then
        if [[ -f "$MANIFEST_FILE" ]] && grep -Fxq "/usr/local/bin/wgcf" "$MANIFEST_FILE" 2>/dev/null; then
            remove_managed_path /usr/local/bin/wgcf || return 1
        else
            _err "检测到非本脚本管理的 /usr/local/bin/wgcf，拒绝覆盖"
            return 1
        fi
    fi
    ...
    for url in "${wgcf_urls[@]}"; do
        ...
        # 捕获详细错误
        local wgcf_tmp
        wgcf_tmp=$(mktemp "${TMPDIR:-/tmp}/vless-wgcf-bin.XXXXXX") || return 1
        last_error=$(curl -fsSL -o "$wgcf_tmp" -A "Mozilla/5.0" --max-redirs 5 --connect-timeout 15 --max-time 90 "$url" 2>&1)
        local curl_ret=$?
        ...
        if [[ $curl_ret -eq 0 ]]; then
            if [[ ! -f "$wgcf_tmp" ]]; then
                echo -e "    ${R}✗ 文件未生成${NC}"
            elif [[ ! -s "$wgcf_tmp" ]]; then
                echo -e "    ${R}✗ 文件为空${NC}"
                rm -f "$wgcf_tmp"
            elif command -v file &>/dev/null; then
                # 有 file 命令：完整验证
                if ! file "$wgcf_tmp" 2>/dev/null | grep -q "ELF"; then
                    echo -e "    ${R}✗ 文件格式错误（非 ELF 可执行文件）${NC}"
                    echo -e "    ${D}文件类型: $(file "$wgcf_tmp" 2>/dev/null)${NC}"
                    rm -f "$wgcf_tmp"
                else
                    chmod +x "$wgcf_tmp"
                    prepare_managed_path "/usr/local/bin/wgcf" || { rm -f -- "$wgcf_tmp"; return 1; }
                    mv "$wgcf_tmp" /usr/local/bin/wgcf
                    echo -e "    ${G}✓ 下载成功${NC}"
                    return 0
                fi
            else
                # 无 file 命令：降级验证（检查文件大小）
                local filesize=$(stat -f%z "$wgcf_tmp" 2>/dev/null || stat -c%s "$wgcf_tmp" 2>/dev/null)
...
register_warp() {
    ...
    local wgcf_workdir
    wgcf_workdir=$(mktemp -d "${TMPDIR:-/tmp}/vless-wgcf.XXXXXX") || return 1
    local old_pwd="$PWD"
    cd "$wgcf_workdir" || { rm -rf -- "$wgcf_workdir"; return 1; }
    ...
    if [[ $register_ret -ne 0 ]] || [[ ! -f "$wgcf_workdir/wgcf-account.toml" ]]; then
        echo -e " ${R}✗${NC}"
        _err "WARP 账户注册失败"
    ...
    if [[ $generate_ret -ne 0 ]] || [[ ! -f "$wgcf_workdir/wgcf-profile.conf" ]]; then
        echo -e " ${R}✗${NC}"
        _err "配置生成失败"
    ...
    # 解析配置并保存到 JSON
    echo -ne "  ${C}▸${NC} 保存配置..."
    parse_and_save_warp_config "$wgcf_workdir/wgcf-profile.conf"
    cd "$old_pwd" 2>/dev/null || true
    rm -rf -- "$wgcf_workdir"
    echo -e " ${G}✓${NC}"
...
refresh_warp_wgcf() {
    ...
    # 删除旧配置
    rm -f "$WARP_CONF_FILE"
    remove_managed_path /usr/local/bin/wgcf
...
install_warp_official() {
    ...
    if [[ "$DISTRO" == "ubuntu" || "$DISTRO" == "debian" ]]; then
        ...
        # 添加 GPG 密钥；仅认领本脚本新创建的文件
        prepare_managed_path /usr/share/keyrings/cloudflare-warp-archive-keyring.gpg || return 1
        ...
        prepare_managed_path /etc/apt/sources.list.d/cloudflare-client.list || return 1
        ...
        prepare_managed_path /etc/yum.repos.d/cloudflare-warp.repo || return 1
    ...
    while [[ $svc_retry -lt 5 ]]; do
        ...
        if systemctl is-active warp-svc &>/dev/null; then
            echo -e " ${G}✓${NC}"
            echo ""
            touch "$CFG/.owned-warp-official"
            chmod 600 "$CFG/.owned-warp-official" 2>/dev/null || true
            _ok "WARP 官方客户端安装成功"
            return 0
        fi
        ((svc_retry++))
...
uninstall_warp_official() {
    ...
    if [[ ! -f "$CFG/.owned-warp-official" ]]; then
        _warn "当前 WARP 官方客户端不是由本脚本安装，保留软件包和仓库文件"
        return 0
    fi
    ...
    if [[ "$DISTRO" == "ubuntu" || "$DISTRO" == "debian" ]]; then
        ...
        remove_managed_path /etc/apt/sources.list.d/cloudflare-client.list
        remove_managed_path /usr/share/keyrings/cloudflare-warp-archive-keyring.gpg
    elif [[ "$DISTRO" == "centos" ]]; then
        yum remove -y cloudflare-warp 2>/dev/null
        remove_managed_path /etc/yum.repos.d/cloudflare-warp.repo
    fi

    rm -f -- "$CFG/.owned-warp-official"
    _ok "WARP 官方客户端已卸载"
}
...
_display_sorted_latencies() {
    ...
    sort -t'|' -k1 -n "$results" | while IFS='|' read -r _ latency name type server port; do
        ...
        if [[ -n "$marks_array_name" ]]; then
            local -n marks_ref="$marks_array_name"
            local mark_value="${marks_ref[$name]:-}"
...
uninstall_specific_protocol() {
    ...
    if [[ " $XRAY_PROTOCOLS " == *" $selected_protocol "* ]]; then
        ...
        if [[ -n "$remaining_singbox" ]]; then
            ...
            if [[ "$DISTRO" == "alpine" ]]; then
                ...
                remove_managed_path "/etc/init.d/vless-singbox"
            else
                remove_managed_path "/etc/systemd/system/vless-singbox.service"
                systemctl daemon-reload
            fi
            _ok "Sing-box 服务已停止"
        fi
        ...
        if [[ "$DISTRO" == "alpine" ]]; then
            ...
            remove_managed_path "/etc/init.d/$service_name"
            # ShadowTLS 后端服务
            if [[ -n "${BACKEND_NAME[$selected_protocol]:-}" ]]; then
                rc-update del "${BACKEND_NAME[$selected_protocol]}" default 2>/dev/null
                remove_managed_path "/etc/init.d/${BACKEND_NAME[$selected_protocol]}"
            fi
        else
            systemctl disable "$service_name" 2>/dev/null
            remove_managed_path "/etc/systemd/system/${service_name}.service"
            # ShadowTLS 后端服务
            if [[ -n "${BACKEND_NAME[$selected_protocol]:-}" ]]; then
                systemctl disable "${BACKEND_NAME[$selected_protocol]}" 2>/dev/null
                remove_managed_path "/etc/systemd/system/${BACKEND_NAME[$selected_protocol]}.service"
            fi
            systemctl daemon-reload
        fi
    fi
    ...
    if [[ "$has_sub_protocol" == "false" ]]; then
        ...
        remove_managed_path /etc/nginx/conf.d/vless-sub.conf
        remove_managed_path /etc/nginx/http.d/vless-sub.conf
        remove_managed_path /etc/nginx/conf.d/vless-fake.conf
        remove_managed_path /etc/nginx/http.d/vless-fake.conf
...
do_uninstall() {
    ...
    if [[ "$warp_st" == "configured" || "$warp_st" == "connected" ]] || check_cmd warp-cli; then
        ...
        if [[ "$warp_mode" == "official" && -f "$CFG/.owned-warp-official" ]]; then
            # 仅卸载由本脚本安装的官方客户端
            warp-cli disconnect 2>/dev/null
            systemctl stop warp-svc 2>/dev/null
            systemctl disable warp-svc 2>/dev/null
        ...
        rm -f "$CFG/.owned-warp-official" 2>/dev/null || true
        _ok "WARP 已卸载"
    fi
    ...
    if command -v systemctl >/dev/null 2>&1 && systemctl is-active --quiet fake-web 2>/dev/null; then
        ...
        remove_managed_path /etc/systemd/system/fake-web.service
        systemctl daemon-reload 2>/dev/null
        cleaned_items+=("fake-web服务")
    fi
    
    # Nginx 配置文件由安装清单精确删除；这里只在删除后尝试安全重载。
    local nginx_was_present=false
    command -v nginx >/dev/null 2>&1 && nginx_was_present=true
    ...
    # 清理网页文件
    remove_managed_path /var/www/html/index.html
    
    # 强力清理残留进程
    force_cleanup
    
    _info "删除本脚本安装清单中的系统文件..."
    remove_managed_paths
    if [[ "$DISTRO" != "alpine" ]]; then
        systemctl daemon-reload 2>/dev/null || true
    fi
    if [[ "$nginx_was_present" == "true" ]] && nginx -t >/dev/null 2>&1; then
        svc reload nginx 2>/dev/null || true
    fi
    ...
    cert_backup_dir=$(mktemp -d "${TMPDIR:-/tmp}/vless-certs-backup.XXXXXX") || return 1
    if [[ -d "$CFG/certs" ]]; then
        _info "备份证书文件..."
    ...
    rm -rf -- "$cert_backup_dir" 2>/dev/null || true
...
do_install_server() {
        ...
        if [[ "$core" != "standalone" ]]; then
            ...
            if [[ "$DISTRO" == "alpine" ]]; then
                ...
                if [[ -n "${BACKEND_NAME[$protocol]:-}" ]]; then
                    ...
                    remove_managed_path "/etc/init.d/${BACKEND_NAME[$protocol]}"
                fi
            else
                systemctl disable "$service_name" 2>/dev/null
                remove_managed_path "/etc/systemd/system/${service_name}.service"
                ...
                if [[ -n "${BACKEND_NAME[$protocol]:-}" ]]; then
                    ...
                    remove_managed_path "/etc/systemd/system/${BACKEND_NAME[$protocol]}.service"
                fi
                systemctl daemon-reload
            fi
    ...
    _info "创建服务..."
    create_server_scripts  # 生成服务端辅助脚本（hy2-nat、tuic-nat）
...
install_cloudflared() {
    ...
    if download_file_checked "$dl_url" "$tmp_file" binary; then
        chmod +x "$tmp_file"
        prepare_managed_path "$CLOUDFLARED_BIN" || { rm -f -- "$tmp_file"; return 1; }
...
_setup_cloudflared_service() {
    ...
    if [[ "$DISTRO" == "alpine" ]]; then
        # OpenRC 服务
        prepare_managed_path "/etc/init.d/$CLOUDFLARED_SERVICE" || return 1
...
EOF
        ...
        # systemd 服务
        prepare_managed_path "/etc/systemd/system/${CLOUDFLARED_SERVICE}.service" || return 1
...
uninstall_cloudflared() {
    ...
    if [[ "$DISTRO" == "alpine" ]]; then
        ...
        remove_managed_path "/etc/init.d/$CLOUDFLARED_SERVICE"
    else
        systemctl stop "$CLOUDFLARED_SERVICE" 2>/dev/null
        systemctl disable "$CLOUDFLARED_SERVICE" 2>/dev/null
        remove_managed_path "/etc/systemd/system/${CLOUDFLARED_SERVICE}.service"
        systemctl daemon-reload
    fi
    ...
    # 删除文件
    _info "清理文件..."
    remove_managed_path "$CLOUDFLARED_BIN"
...
show_logs() {
    ...
    echo -e "  ${G}2${NC}) 查看服务日志 (按协议选择)"
    echo -e "  ${G}3${NC}) 实时跟踪脚本日志"
    echo -e "  ${G}0${NC}) 返回"
    _line
...
install_realm_binary() {
    ...
    tmp=$(mktemp -d)
    download_file_checked "$url" "$tmp/realm.tar.gz" tar.gz || { rm -rf -- "$tmp"; return 1; }
    tar -xzf "$tmp/realm.tar.gz" -C "$tmp" || { rm -rf -- "$tmp"; return 1; }
    local bin=""
    [[ -f "$tmp/realm" ]] && bin="$tmp/realm"
    [[ -f "$tmp/realm-slim" ]] && bin="$tmp/realm-slim"
    [[ -z "$bin" ]] && { _err "Realm 解压后未找到二进制"; rm -rf "$tmp"; return 1; }
    install_managed_file "$bin" "$REALM_BIN" 755
    rm -rf "$tmp"
...
create_realm_service() {
    if [[ "$DISTRO" == "alpine" ]]; then
        prepare_managed_path "/etc/init.d/$REALM_SVC" || return 1
...
EOF
        ...
        prepare_managed_path "/etc/systemd/system/${REALM_SVC}.service" || return 1
...
# 自检：不修改系统，只检查脚本、依赖、权限、数据库和核心状态
self_check() {
    local failed=0 warnings=0 cmd mode
    echo "VLESS 一键脚本自检"
    echo "脚本: $0"
    echo ""
    ...
    if [[ $EUID -eq 0 ]]; then
        ...
        echo "[WARN] 当前不是 root；只影响安装操作"
        warnings=$((warnings + 1))
    fi

    for cmd in bash curl jq awk sed grep ip ss openssl; do
    ...
    if command -v flock >/dev/null 2>&1; then
        ...
        echo "[WARN] 未安装 flock，将使用 mkdir 兼容锁"
        warnings=$((warnings + 1))
    fi

    if [[ -d "$CFG" ]]; then
        mode=$(stat -c '%a' "$CFG" 2>/dev/null || stat -f '%Lp' "$CFG" 2>/dev/null || echo unknown)
        echo "[INFO] 配置目录权限: $mode"
        [[ "$mode" == "700" ]] || { echo "[WARN] 建议配置目录权限为 700"; warnings=$((warnings + 1)); }
    else
        echo "[INFO] 尚未创建配置目录"
    fi

    if [[ -f "$DB_FILE" ]]; then
        if jq empty "$DB_FILE" >/dev/null 2>&1; then
            echo "[OK] 数据库 JSON"
        else
            echo "[FAIL] 数据库 JSON 损坏"
            failed=1
        fi
    else
        echo "[INFO] 尚未创建数据库"
    ...
    if [[ -f "$TG_CONFIG_FILE" ]]; then
        if jq empty "$TG_CONFIG_FILE" >/dev/null 2>&1; then
            echo "[OK] Telegram JSON"
        else
            echo "[FAIL] Telegram JSON 损坏"
            failed=1
        fi
    fi

    for cmd in xray sing-box snell-server snell-server-v5 anytls-server shadow-tls cloudflared realm; do
        if command -v "$cmd" >/dev/null 2>&1; then
            echo "[INFO] 已安装: $cmd"
        fi
    done
    ...
    echo ""
    echo "结果: 失败 $failed 项，警告 $warnings 项"
    [[ $failed -eq 0 ]]
}

# 命令行参数处理
# --help 和 --self-check 不加锁；其他操作串行执行，避免重复运行和数据库并发写入
case "${1:-}" in
    --help|-h|--self-check) ;;
    *)
        acquire_instance_lock || exit 1
        ensure_secure_permissions 2>/dev/null || true
        cleanup_legacy_watchdog
        ;;
esac
